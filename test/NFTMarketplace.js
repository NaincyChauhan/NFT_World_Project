...
describe('NFT PORFOLIO', () => {
    ...
    describe("Deployment", function () {
        ...
        it("Should compare corrent menager account and feePercent", async function () {
            expect(await nft.feePercent()).to.equal(1);
    ...
    describe('NFT Minting', () => {
        ...
        describe('Success', () => {
            beforeEach(async () => {
                transcation = await nft.connect(manager).createNFT(1, "Image URL", 1, false);
            ...
            it("Should Return New NFT Data", async () => {
                ...
                expect(Number(NFTData[0])).to.equal(1);
                expect(Number(NFTData[1])).to.equal(1);
                expect(NFTData[2]).to.equal("Image URL");
                expect(NFTData[3]).to.equal(manager.address);
                expect(NFTData[3]).to.equal(manager.address);
                expect(Number(NFTData[5])).to.equal(1);
                expect(NFTData[6]).to.equal(false);
            });

            it("Should Return User NFTs && and Check User OwnerShip", async () => {
                transcation = await nft.connect(manager).createNFT(1, "Image URL 2 ", 1, false);
                result = await transcation.wait();
                const NFTs = await nft.getNFTsByUser(manager.address);
                for (let index = 0; index < NFTs.length; index++) {
                    expect(NFTs[index].creator).to.equal(manager.address);
                    expect(await NFTs[index].owner).to.equal(manager.address);
                }
            });
        });
        ...
        describe('Failure', () => {
            it("Should Fail when Price is low", async () => {
                await expect(nft.connect(manager).createNFT(1, "Image URi2", 0, true))
    ...
    describe('Create Collection', () => {
        ...
        let collection_id;
        beforeEach(async () => {
            transcation = await nft.connect(manager).createNFT(1, "Image URl 2", 2, false);
            result = await transcation.wait();
    ...
    describe('List NFT', () => {
        ...
        beforeEach(async () => {
            transcation = await nft.connect(manager).createNFT(1, "Image URI", 1, false);
            result = await transcation.wait();
        });
        ...
        describe('Success', () => {
            it("Should List NFT for sale ", async () => {
                ...
                expect(nftData[6]).to.equal(true);
            });
            ...
            it("Should Change The NFT Price ", async () => {
                ...
                result = await transcation.wait();
                nftData = await nft.nfts(1);
                expect(Number(nftData[5])).to.equal(4);
            });
        });
    ...
    describe('Buy NFT', () => {
        ...
        beforeEach(async () => {
            transcation = await nft.connect(manager).createNFT(1, "Image URi", 2, true);
            result = await transcation.wait();
        ...
        describe('Success', () => {
            ...
            it("Should Calculate right balance", async () => {
                let nft_price = Number(nftData[5]);
                let marketPlaceFee = (nft_price * 1) / 100;
        ...
        describe('Failure', () => {
            ...
            it("Should return when price is less then nft price", async () => {
                const new_nft = await nft.connect(add2).createNFT(1, "Image URi", 3, false);
    ...
    describe('Get NFT Data By Pagination', () => {
        beforeEach(async () => {
            const _metadata = "QmVHVYYgDhmKNkkcusyRoBfREnzM1DySouKv6PQWGMSotG";
            const createCollection = await nft.connect(add3).createCollection(_metadata, "Collection First");
            ...
            for (let index = 1; index < 31; index++) {
                ...
                const createNFT = await nft.connect(add3).makeCollectionNFT(mediaType, _nft_metadata, index, true, 1);
